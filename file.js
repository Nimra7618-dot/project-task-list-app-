...
window.addEventListener("DOMContentLoaded", function () {
   ...
   let dynamicDivs = Array.from(tasksDiv.querySelectorAll(".task-card")).slice(savedTasks.length);
...
let hardcodedDivs = tasksDiv.querySelectorAll(".task-card h3");
    hardcodedDivs.forEach((h3, i) => {
        if (savedTasks[i]) h3.innerText = savedTasks[i];
        let checkbox = h3.closest(".task-card").querySelector("input[type=checkbox]");
        if (checkbox) {
...
function moveTask(box) {
    let taskDiv = box.closest(".task-card");
...
function createTaskDiv(taskName, isCompleted) {
    ...
    newDiv.className = "task-card"; // changed from "task-box"
    newDiv.innerHTML = `
        <div class="task-left">
            <h3>${taskName}</h3>
        </div>
...
function moveTask(box) {
    ...
    if (box.checked) {
        if (!completedTasks.includes(taskName)) completedTasks.push(taskName);
    } else {
        completedTasks = completedTasks.filter(t => t !== taskName);
    }
...
function addToCompletedSection(taskName) {
    ...
    // Avoid duplicates
    if (Array.from(completedDiv.getElementsByTagName("p")).some(p => p.innerText.includes(taskName))) return;
...
function editTask(btn) {
    currentTaskDiv = btn.closest(".task-card"); // was ".task-box"
...
function deleteTask(btn) {
    ...
    let taskDiv = btn.closest(".task-card"); // was ".task-box"
    let taskName = taskDiv.querySelector("h3").innerText;
    taskDiv.remove();
...
function deleteTask(btn) {
    ...
    let taskDiv = btn.closest(".task-card");
    let taskName = taskDiv.querySelector("h3").innerText;
    taskDiv.remove();
